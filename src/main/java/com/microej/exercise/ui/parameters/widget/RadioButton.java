...
public class RadioButton extends Widget {
	...
	/**
	 * Creates a radio button with the given text to display and an extra locale attribute
	 *
	 * @param locale
	 * 			the locale associated with the button
	 * @param text
	 *          the text to display.
	 * @param group
	...
	public boolean handleEvent(int event) {
		...
		if (type == Pointer.EVENT_TYPE) {
			...
			if (action == Buttons.RELEASED) {
				 ...
				 *
				 * Step 12.4: Update the locale when interacting with the RadioButton
				 *
				 * 1. Set the current locale every time a button is clicked by calling the correct Model method.
				 *
				 */
