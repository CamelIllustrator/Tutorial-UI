...
import com.microej.exercise.ui.parameters.ParametersPage;
...
public class ApplicationMenuPage extends Page {
	...
	public Widget getWidget() {
		...
		// adds the parameters page as the second one in the list
		addParametersPage(list);

		// populates the rest of the list with stub applications
		int applicationsCount = APPLICATIONS_NAMES.length;
		for (int i = 2; i < applicationsCount; i++) {
	...
	private void addParametersPage(ApplicationList list) {
		OnClickListener listener = new OnClickListener() {

			@Override
			public void onClick() {
				// shows the parameters page when the user clicks on the first item
				Main.showPage(new ParametersPage());
			}
		};
		ApplicationListItem parametersItem = new ApplicationListItem(APPLICATIONS_ICONS[1], APPLICATIONS_NAMES[1],
				listener);
		list.addListItem(parametersItem);
	}
